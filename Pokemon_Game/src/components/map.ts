  ...
  isSolidTileAtXY(x: number, y: number, dirX: number, dirY: number): boolean {
    ...
    return this.currentMap.layers.reduce((res: boolean, layer: number[], index: number) => {
      ...
      const oneWay: boolean =
        (tile === 3 && rowHalfTile * constants.MAP_TSIZE < y && (dirY === -1 || dirX !== 0)) ||
        (tile === 4 && rowHalfTile * constants.MAP_TSIZE < y && (dirY === -1 || dirX === -1)) ||
        (tile === 7 && rowHalfTile * constants.MAP_TSIZE < y && (dirY === -1 || dirX === 1)) ||
        (tile === 5 && rowHalfTile * constants.MAP_TSIZE < y && colHalfTile * constants.MAP_TSIZE < x) ||
        (tile === 8 && rowHalfTile * constants.MAP_TSIZE < y && colHalfTile * constants.MAP_TSIZE > x) ||

        (tile === 6 && 
          (rowHalfTile * constants.MAP_TSIZE < y || colHalfTile * constants.MAP_TSIZE < x) && 
          (dirX === -1 || dirY === -1)
        ) ||
        ...
        (tile === 33 && colHalfTile * constants.MAP_TSIZE < x && (dirX === -1 || dirY !== 0)) ||
        (tile === 34 && colHalfTile * constants.MAP_TSIZE < x && (dirX === -1 || dirY === 1));
        (tile === 35 && 
          (rowHalfTile * constants.MAP_TSIZE < y  ||
