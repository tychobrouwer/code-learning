<?php
    ...
    $getUser = json_decode(file_get_contents("https://custom-xija.frb.io/getStats/getAllUserData.php?name=$nameInput&platform=$platform&region=$region&appcode=809965"), true);

    // Check If Player Was Found
if (!empty($getUser['players'])) {
    // Get PLayer ID
    $playerId = key($getUser['players']);

    // Merge getUser and getUserStats arrays (Duplicate Stats Get Overritten)
    $userData = $getUser['players'][$playerId];
